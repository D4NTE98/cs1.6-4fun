...
public plugin_init() { 
	...
	CVAR_HIGHSPEED = register_cvar("km_highspeed","220")
	...
	CVAR_LOWGRAV = register_cvar("km_lowgravity" , "600")
...
public plugin_precache() { 
	precache_model("models/4fun_knife/v_butcher.mdl") 
	precache_model("models/4fun_knife/p_butcher.mdl") 
	precache_model("models/4fun_knife/v_machete.mdl")
	precache_model("models/4fun_knife/p_machete.mdl")
	precache_model("models/4fun_knife/v_bak.mdl")
	precache_model("models/4fun_knife/p_bak.mdl")
	precache_model("models/4fun_knife/v_pocket.mdl")
	precache_model("models/4fun_knife/p_pocket.mdl")
...
public display_knife(id) {
	new menuBody[512]
	add(menuBody, 511, "\r[4FUN] \yWybierz kose \w^n^n")
	add(menuBody, 511, "1. \rMaczeta \d» \w+25% dmg ^n")
	add(menuBody, 511, "2. \rWojskowy \d» \wbrak kroków^n")
	add(menuBody, 511, "3. \rScyzoryk \d» \w+25% szybkosci ^n")
	add(menuBody, 511, "4. \rKuchenny \d» \w-20% grawitacji ^n")
	add(menuBody, 511, "5. \rDomyslny \d» \wregeneracja HP^n^n")
	add(menuBody, 511, "0. \dAnuluj^n")
...
public SetKnife(id , Knife) {
	...
	switch(Knife)
	{
		...
		case 1: {
			format(vModel,55,"models/4fun_knife/v_butcher.mdl")
			format(pModel,55,"models/4fun_knife/p_butcher.mdl")
		}
		case 2: {
			format(vModel,55,"models/4fun_knife/v_bak.mdl")
			format(pModel,55,"models/4fun_knife/p_bak.mdl")
		}
		case 3: {
			format(vModel,55,"models/4fun_knife/v_pocket.mdl")
			format(pModel,55,"models/4fun_knife/p_pocket.mdl")
		}
		case 4: {
			format(vModel,55,"models/4fun_knife/v_machete.mdl")
			format(pModel,55,"models/4fun_knife/p_machete.mdl")
		}
	} 
...
public kmodmsg() { 
	...
	client_print(0,print_chat,"[4FUN] Wpisz /kosa aby wybrac typ kosy!")
}  
